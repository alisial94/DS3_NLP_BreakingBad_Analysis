...
- *_Does the character of Jesse Pinkman becomes positive, while the character for Walter White becomes more negative as the show progresses_*
- _*How does the topic/theme associated to each character develops/changes as the show progresses*_



# Data
If you are a fan then you would know that series has a total of 5 seasons. Unfortunately, the transcripts data available online has labels attached to each dialog until episode 6 of season 3. I exhausted all the other resources to try and get the transcript with labels for the remaining episodes, but I wasn't able to find any resource apart from few original text pdf of the screenplay. After reviewing that document before converting into text, I realsied that there where barely any dialog and those PDFs were mainly focused on setting the scene for each act, which is not what I was looking for. Therefore, I made a conscious decision to work with the data I have. 

The data at hand is directly scrapped from _[Forever Dreaming](https://transcripts.foreverdreaming.org/viewforum.php?f=165)_. The R script I used to scrape this data has been uploaded to my GitHub as _[scrapping_bb_data](https://github.com/alisial94/DS3_NLP_BreakingBad_Analysis/blob/main/Code/scrapping_bb_data.R)_ (please link to access it directly). The data has about 5596 dialogs (observations) in total with 5 variables which are:
- actor
...
As I mentioned that we only have two and a hlaf seasons worth of transcripts to perform this analysis, therefore, the two hypothsis stated above would still based seasons, but just the first 3. As you can see below the graphs indicate how many line there are in each episode and the charts divided based on each season.
...
Based on the graph above we see that at this point in the breaking bad series, the most important characters are Walter and Jesse and Walter's family (which includes Hank). 


# Most Frequent Words
Before moving on towards few further alterations in the dataset, I decided to have a look at the most commonly used words in the series. 
...
Based on the graph above it is evident that the data we have, basically the script itself mainly uses a lot of come words which makes sense because when having a conversation you tend to use similar words. Before, I constructed this the word cloud and chart for top 20 words, I manually added few additional words to the stop words after careful consideration. One thing I realised during this exercise that most of the swear words, specially the one Jesse uses have been not included in the script, which is not very helpful because that would have helped us a lot when we move to perform sentiment analysis. But as a mentioned earlier, we have to work with what we have. 
...
# Sentiment Analysis
Now that we explored are data and gather a decent insight about are characters, let's swing back to the original hypothesis. In this section we will be looking at how the character changes as the show progresses. The analysis will only be focusing on the two main characters of the show as we discussed earlier and characters are Walter and Jesse. I will be using three different approaches to try and understand the sentiments. 


## BING Lexicon

Bing Lexicon cathegorizes the words into positives and negatives. First look at general positive and negative words associated to the character of Walter. The word cloud below depicts a very reliable and acceptable categorization of sentiments for the character. Just a side note, the cloud also shows us the character of Walter has more negative words comapred to positive. 
...
I also used BING lexicon to check over all sentiment of each season for both characters. The first cord diagram below is for Walter and appears that the use of more negative words is increasing as we move from Season 1 to Season 2. One might question that since we have data for only half of the series and the amount of episodes in the Season 2 are more than compared to Season 1, thus the results are not conclusive. But when you observe closely, the weight of negative sentiments is higher compared to the weight positive sentiments is Season 2. On the hand, the in Season 1 the wights of sentiment are quite close to each other. Thus, based on these, we can say that character of Walter is becoming more negative as the show progresses. 
...
Now Let's look at how these sentiments ranks fit in our data. First we look at the overall mood of the character. As you can from the graph below we get a basic idea about the frequency of each for the characters up until now in the series, but lets dig a little deeper. 
...
For this sentiment model, we will be only looking at Season 1 and Season 2, since comparing those is much easier and we have very less data on Season 3. First we look at how Walter's character performed against Afinn ranking. From the charted provide below, unfortunately we don't get much insight since the ratio of positive to negative sentiments is similar in both the seasons, therefore in case of Walter Afinn provided us with more neutral result which doesn't help understanding if the character is becoming more negative. 
...
```
Again, when I used Afinn to understand the trend of Jesse's character, it did not produce any conclusive results. In fact, Afinn result summarize Jesse's character more negative then the two sentiment techniques we previously used. 
...
I wanted to understand what these topics are so I decide to look at most frequent words in the clusters for both Walter and Jesse. Based on the graphs provided below for Walter and Jesse, the words used in the cluster are quite similar to what we initially saw as positive or negative word for each of these characters. Therefore, we can say that using LDA we did get a basic understanding that topic association to both the character does change as the show progresses. 
